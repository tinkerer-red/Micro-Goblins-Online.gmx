///scr_sprite_explode(Max Life, Min Life, Max Speed, Min Speed, Max Peice Size, Min Peice Size, Spin, Fade Out, Fade Amt, Fade To Color, End Color, Color Fade Speed, Implode, Sprite);
...
if !to_much_lag(){
  ...
  */
  ww = sprite_get_width(argument13);
  hh = sprite_get_height(argument13);
  ...
  */
  xoff = sprite_get_xoffset(argument13);
  yoff = sprite_get_yoffset(argument13);
  
  
  // Create the particle
  for (i = 0; i < ww; i += argument4) {
      for (j = 0; j < hh; j += argument4) {
          particle = instance_create((x - xoff) + i, (y - yoff) + j, obj_explode_particle);  // Create the particle
