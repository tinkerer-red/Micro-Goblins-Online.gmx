...
if(!instance_exists(_checker)) instance_create(0,0,_checker); 
  ...
  var res = 4,
      margin = 4,
     _x1 = bbox_left + (argument0 - x) -margin,
     _y1 = bbox_top + (argument1 - y) -margin,
     _x2 = bbox_right + (argument0 - x) +margin,
     _y2 = bbox_bottom + (argument1 - y) +margin;
    
    
for(var _x = _x1; _x <= _x2; _x += res){
  for(var _y = _y1; _y <= _y2; _y += res){
