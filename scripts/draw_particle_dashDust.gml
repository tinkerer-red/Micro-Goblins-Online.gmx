///draw_particle_dashDust(x1, y1, x2, y2 [time_min, time_max])
...
if instance_exists(obj_particle_controller){
  ...
  var array = dir_to_dir_and_flip(point_direction(x1,y1, x2, y2))
  var _dir = array[0]
  var flip_point = array[1]
  
  var emit_time = 1
  
  //the particle type's string, used for key name, and checks
  var part_string = "part_type_dashDust_"+string(x1 div 64)+string(y1 div 64)
  var emit_string = "part_emit_dashDust_"+string(x1 div 64)+"_"+string(y1 div 64)
  ...
  if (ds_map_find_value(obj_particle_controller.particle_map, part_string) = undefined){
    ...
    part_type_scale(part_type, 1, flip_point);
